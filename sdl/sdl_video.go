...
const (
	WindowPosUndefinedMask uint32 = 0x1FFF0000             // A magic value used with [WindowPosUndefined].
	WindowPosUndefined            = WindowPosUndefinedMask // Used to indicate that you don't care what the window position/display is.
	WindowPosCenteredMask  uint32 = 0x2FFF0000             // A magic value used with [WindowPosCentered].
	WindowPosCentered             = WindowPosCenteredMask  // Used to indicate that the window position should be centered.
)

// WindowPosUndefinedDisplay marks window position to be "undefined" and returns modified value.
func WindowPosUndefinedDisplay(displayID DisplayID) uint32 {
	return WindowPosUndefinedMask | uint32(displayID)
}

// WindowPosIsUndefined tests if the window position is marked as "undefined".
func WindowPosIsUndefined(displayID DisplayID) bool {
	return (uint32(displayID) & 0xFFFF0000) == WindowPosUndefinedMask
}

// WindowPosCenteredDisplay marks window position to be "centered" and returns modified value.
func WindowPosCenteredDisplay(displayID DisplayID) uint32 {
	return WindowPosCenteredMask | uint32(displayID)
}

// WindowPosIsCentered tests if the window position is marked as "centered".
func WindowPosIsCentered(displayID DisplayID) bool {
	return (uint32(displayID) & 0xFFFF0000) == WindowPosCenteredMask
}
...
// [EGLDisplay] is an opaque type for an EGL display.
//
// [EGLDisplay]: https://wiki.libsdl.org/SDL3/SDL_EGLDisplay
type EGLDisplay struct{}

// [EGLConfig] is an opaque type for an EGL config.
//
// [EGLConfig]: https://wiki.libsdl.org/SDL3/SDL_EGLConfig
type EGLConfig struct{}

// [EGLSurface] is an opaque type for an EGL surface.
//
// [EGLSurface]: https://wiki.libsdl.org/SDL3/SDL_EGLSurface
type EGLSurface struct{}

// [EGLAttrib] defines an EGL attribute, used when creating an EGL context.
//
// [EGLAttrib]: https://wiki.libsdl.org/SDL3/SDL_EGLAttrib
type EGLAttrib uintptr

// [EGLint] defines an EGL integer attribute, used when creating an EGL surface.
//
// [EGLint]: https://wiki.libsdl.org/SDL3/SDL_EGLint
type EGLint int32

// [EGLAttribArrayCallback] defines an EGL platform attribute initialization callback.
//
// [EGLAttribArrayCallback]: https://wiki.libsdl.org/SDL3/SDL_EGLAttribArrayCallback
type EGLAttribArrayCallback *EGLAttrib

// [EGLIntArrayCallback] defines an EGL surface/context attribute initialization callback types.
//
// [EGLIntArrayCallback]: https://wiki.libsdl.org/SDL3/SDL_EGLIntArrayCallback
type EGLIntArrayCallback *EGLint
